...
/*
Plugin Name: Kittify
Plugin URI: https://github.com/iedimitrov/wp-for-dev.git
...
Version: 1.0
Author URI: http://cloudskyhigh.net
*/

function kittify_get_lines() {
	/* These are the funny quotes, meant to kittify you */
...
function kittify() {
	...
	printf(
		'<p id="kittify"><span class="screen-reader-text">%s </span><span dir="ltr"%s>%s</span></p>',
		__( 'Kittifying quotes, by Ivan Dimitrov:' ),
		$lang,
		$chosen
	);
}
...
add_action( 'admin_notices', 'kittify' );

// We need some CSS to position the paragraph.
function kittify_css() {
	echo "
	<style type='text/css'>
	#kittify {
		float: right;
...
add_action( 'admin_head', 'kittify_css' );
